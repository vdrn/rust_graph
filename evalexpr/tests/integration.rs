...
fn test_pow_examples() {
    ...
    assert_eq!(
        eval("6 ^ 4"),
        Ok(Value::Float((6.0 as DefaultNumericTypes).powf(4.0)))
    );
    assert_eq!(eval("1 ^ 4 + 2"), Ok(Value::Float(3.0)));
...
fn test_with_context() {
    ...
    assert_eq!(
        context.remove_value(istr("half")),
        Ok(Some(Value::Float(0.5)))
    );
    assert_eq!(
        context.remove_value(istr("zero")),
...
fn test_capturing_functions() {
    ...
    assert_eq!(
        eval_with_context("mult_3(3)", &context),
        Ok(Value::Float(9.0))
    );
    assert_eq!(
...
fn test_builtin_functions() {
    ...
    assert_eq!(
        eval("math::exp(2)"),
        Ok(Value::Float((2.0 as DefaultNumericTypes).exp()))
    );
    assert_eq!(
        eval("math::exp2(2)"),
        Ok(Value::Float((2.0 as DefaultNumericTypes).exp2()))
    );
    assert_eq!(
        eval("math::pow(1.5, 1.3)"),
        Ok(Value::Float((1.5 as DefaultNumericTypes).powf(1.3)))
    );
    // Cos
    ...
    assert_eq!(
        ...
        Ok(Value::Float((8.2 as DefaultNumericTypes).hypot(1.1)))
    );
    // Rounding
...
fn test_shortcut_functions() {
    ...
    assert_eq!(
        ...
        Err(EvalexprError::ExpectedEmpty {
            actual: Value::Tuple(thin_vec![Value::Empty, Value::Empty])
        })
    );
    assert_eq!(
        eval_empty("xaq"),
...
fn test_tuple_definitions() {
    ...
    assert_eq!(
        eval_tuple("(3, 4)"),
        Ok(thin_vec![Value::from_float(3.0), Value::from_float(4.0)])
    );
    assert_eq!(
        eval_tuple("2, (5, 6)"),
        Ok(thin_vec![
    ...
    assert_eq!(
        eval_tuple("1, 2"),
        Ok(thin_vec![Value::from_float(1.0), Value::from_float(2.0)])
    );
    assert_eq!(
        eval_tuple("1, 2, 3, 4"),
        Ok(thin_vec![
    ...
    assert_eq!(
        eval_tuple("1, ()"),
        Ok(thin_vec![Value::from_float(1.0), Value::from(())])
    );
    assert_eq!(
        eval_tuple("1, ((2))"),
        Ok(thin_vec![Value::from_float(1.0), Value::from_float(2.0)])
...
fn test_operator_assignments() {
    ...
    let mut context = HashMapContext::<DefaultNumericTypes>::new();
    assert_eq!(
        eval_float_with_context_mut("a = 5; a", &mut context),
        Ok(5.0)
    );
    assert_eq!(
        eval_float_with_context_mut("a += 3; a", &mut context),
        Ok(8.0)
    );
    assert_eq!(
        eval_float_with_context_mut("a -= 5; a", &mut context),
...
fn test_error_constructors() {
    ...
    assert_eq!(
        Value::Tuple(thin_vec![
            Value::<DefaultNumericTypes>::Float(4.0),
            Value::Float(5.0)
        ])
...
fn test_value_type() {
    ...
    assert_eq!(
        ValueType::from(&Value::<DefaultNumericTypes>::Tuple(Default::default())),
        ValueType::Tuple
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Err(EvalexprError::ExpectedFloat {
            actual: Value::Tuple(Default::default())
        })
    );
    assert_eq!(
    ...
    assert_eq!(
        Value::<DefaultNumericTypes>::Tuple(Default::default()).as_tuple(),
        Ok(Default::default())
    );
    assert_eq!(
