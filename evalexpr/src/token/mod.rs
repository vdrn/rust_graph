use crate::{
    ...
    value::numeric_types::{default_numeric_types::DefaultNumericTypes, EvalexprFloat},
};
        ...
        match first {
            ...
            PartialToken::Literal(literal) => {
                ...
                fn litral_to_token<F: EvalexprFloat>(
                    ...
                    if let Ok(number) = parse_dec_or_hex::<F>(&literal) {
                        ...
                        match (second, third) {
                            ...
                            {
                                if let Ok(number) =
                                    format!("{}{}{}", literal, second, third).parse::<F>()
                                {
                ...
                if let Some((token, c)) = litral_to_token::<NumericTypes>(&literal, second, third) {
                    ...
                    if !before.is_empty() {
                        if let Some((token, _)) =
                            litral_to_token::<NumericTypes>(before, None, None)
                        {
                            result.push(token);
                        } else {
