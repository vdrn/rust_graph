	...
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		...
		match self {
			...
			WrongFunctionArgumentAmount { expected, actual } => {
				...
				write!(f, "A function expected {} arguments, but got {}.", expected_arguments, actual)
			},
			...
			ExpectedFixedLengthTuple { expected_length, actual } => {
				write!(f, "Expected a Value::Tuple of length {}, but got {:?}.", expected_length, actual)
			},
			ExpectedRangedLengthTuple { expected_length, actual } => write!(
				f,
			...
			PrecedenceViolation => write!(f, "Tried to append a node to another node with higher precedence."),
			VariableIdentifierNotFound(identifier) => {
				write!(f, "Variable identifier is not bound to anything by context: {:?}.", identifier)
			},
			FunctionIdentifierNotFound(identifier) => {
				write!(f, "Function identifier is not bound to anything by context: {:?}.", identifier)
			},
			...
			MissingOperatorOutsideOfBrace => write!(
				f,
				"Found an opening parenthesis that is preceded by something that does not take any arguments \
				 on the right, or found a closing parenthesis that is succeeded by something that does not \
				 take any arguments on the left."
			),
			UnmatchedPartialToken { first, second } => {
				if let Some(second) = second {
					write!(f, "Found a partial token '{}' that should not be followed by '{}'.", first, second)
				} else {
					write!(
						f,
