...
//!
//! The module also contains some helper functions starting with `expect_` that check for a condition and
//! return `Err(_)` if the condition is not fulfilled. They are meant as shortcuts to not write the same error
...
pub enum EvalexprError<NumericTypes: EvalexprFloat = DefaultNumericTypes> {
	...
	/// Only use this if there is no other error that describes the expected and provided types in more
	/// detail.
	TypeError {
		/// The expected types.
	...
	/// Left of an opening brace or right of a closing brace is a token that does not expect the brace next to
	/// it. For example, writing `4(5)` would yield this error, as the `4` does not have any operands.
	...
	UnmatchedPartialToken {
		...
		/// The token that follows the unmatched partial token and that cannot be matched to the partial
		/// token, or `None`, if `first` is the last partial token in the stream.
...
impl<NumericTypes: EvalexprFloat> EvalexprError<NumericTypes> {
	...
	pub fn wrong_function_argument_amount(actual: usize, expected: usize) -> Self {
		EvalexprError::WrongFunctionArgumentAmount { actual, expected: expected..=expected }
	}

	/// Construct a `WrongFunctionArgumentAmount` error for a function with a range of possible amounts of
	/// arguments.
	...
	pub fn expected_string(actual: Value<NumericTypes>) -> Self { EvalexprError::ExpectedString { actual } }
	...
	pub fn expected_boolean(actual: Value<NumericTypes>) -> Self { EvalexprError::ExpectedBoolean { actual } }

	/// Constructs `EvalexprError::ExpectedTuple{actual}`.
	pub fn expected_tuple(actual: Value<NumericTypes>) -> Self { EvalexprError::ExpectedTuple { actual } }
	...
	pub fn expected_fixed_len_tuple(expected_len: usize, actual: Value<NumericTypes>) -> Self {
		EvalexprError::ExpectedFixedLengthTuple { expected_length: expected_len, actual }
	}

	/// Constructs `EvalexprError::ExpectedFixedLenTuple{expected_len, actual}`.
	pub fn expected_ranged_len_tuple(
		expected_len: RangeInclusive<usize>, actual: Value<NumericTypes>,
	) -> Self {
		EvalexprError::ExpectedRangedLengthTuple { expected_length: expected_len, actual }
	}

	/// Constructs `EvalexprError::ExpectedEmpty{actual}`.
	pub fn expected_empty(actual: Value<NumericTypes>) -> Self { EvalexprError::ExpectedEmpty { actual } }

	/// Constructs an error that expresses that the type of `expected` was expected, but `actual` was found.
	pub(crate) fn expected_type(expected: &Value<NumericTypes>, actual: Value<NumericTypes>) -> Self {
	...
	pub(crate) fn unmatched_partial_token(
		first: PartialToken<NumericTypes>, second: Option<PartialToken<NumericTypes>>,
	) -> Self {
...
pub(crate) fn expect_operator_argument_amount<NumericTypes: EvalexprFloat>(
...
impl<NumericTypes: EvalexprFloat> std::error::Error for EvalexprError<NumericTypes> {}

/// Standard result type used by this crate.
pub type EvalexprResult<T, NumericTypes = DefaultNumericTypes> = Result<T, EvalexprError<NumericTypes>>;
...
mod tests {
	...
	fn trivial_coverage_tests() {
		...
		// );
		assert_eq!(
			EvalexprError::expected_type(&Value::<DefaultNumericTypes>::Boolean(false), Value::Empty),
			EvalexprError::expected_boolean(Value::Empty)
		);
		assert_eq!(
			EvalexprError::expected_type(
				&Value::<DefaultNumericTypes>::Tuple(Default::default()),
				Value::Empty
			),
