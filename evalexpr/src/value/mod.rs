...
use crate::flat_node::cold;
use crate::EvalexprFloat;
use std::convert::TryFrom;
use std::ops::RangeInclusive;

use self::numeric_types::default_numeric_types::DefaultNumericTypes;
...
pub type TupleType<NumericTypes = DefaultNumericTypes> = thin_vec::ThinVec<Value<NumericTypes>>;
...
#[repr(align(16))]
pub enum Value<NumericTypes: EvalexprFloat = DefaultNumericTypes> {
	// /// A string value.
	// String(String),
	/// A float value.
	Float(NumericTypes),
	/// 2 float values.
	Float2(NumericTypes, NumericTypes),
	// /// An integer value.
...
impl<NumericTypes: EvalexprFloat> Value<NumericTypes> {
	...
	pub fn is_float2(&self) -> bool { matches!(self, Value::Float2(_, _)) }
	...
	pub fn as_float(&self) -> EvalexprResult<NumericTypes, NumericTypes> {
		match self {
			Value::Float(f) => Ok(*f),
			value => {
				cold();
				Err(EvalexprError::expected_float(value.clone()))
			},
		}
	}
	/// Clones the value stored in  `self` as `(FloatType,FloatType)`, or returns `Err` if `self` is not a
	/// `Value::Float2`.
	...
	// /// Clones the value stored in  `self` as `FloatType`, or returns `Err` if `self` is not a
	// `Value::Float` or `Value::Int`. /// Note that this method silently converts `IntType` to `FloatType`,
	// if `self` is a `Value::Int`. pub fn as_number(&self) -> EvalexprResult<NumericTypes::Float,
	// NumericTypes> {     match self {
	...
	/// unwraps the value stored in `self` as `TupleType`, or returns `Err` if `self` is not a `Value::Tuple`.
	pub fn into_tuple(self) -> EvalexprResult<TupleType<NumericTypes>, NumericTypes> {
		match self {
			Value::Tuple(tuple) => Ok(tuple),
			value => Err(EvalexprError::expected_tuple(value)),
		}
	}
	/// Returns a reference to the value stored in `self` as `TupleType`, or returns `Err` if `self` is not a
	/// `Value::Tuple`.
	...
	/// Clones the value stored in `self` as `TupleType` or returns `Err` if `self` is not a `Value::Tuple` of
	/// the required length.
	...
	/// Returns the reference to the value stored in `self` as `TupleType` or returns `Err` if `self` is not a
	/// `Value::Tuple` of the required length.
	...
	pub fn as_ranged_len_tuple(
		&self, range: RangeInclusive<usize>,
	) -> EvalexprResult<TupleType<NumericTypes>, NumericTypes> {
		match self {
			Value::Tuple(tuple) => {
				if range.contains(&tuple.len()) {
					...
					Err(EvalexprError::expected_ranged_len_tuple(range, self.clone()))
				}
			},
			value => Err(EvalexprError::expected_tuple(value.clone())),
		}
	...
	pub fn from_float(float: NumericTypes) -> Self { Self::Float(float) }
...
impl<NumericTypes: EvalexprFloat> From<bool> for Value<NumericTypes> {
	fn from(boolean: bool) -> Self { Value::Boolean(boolean) }
}

impl<NumericTypes: EvalexprFloat> From<TupleType<NumericTypes>> for Value<NumericTypes> {
	fn from(tuple: TupleType<NumericTypes>) -> Self { Value::Tuple(tuple) }
}

impl<NumericTypes: EvalexprFloat> From<Value<NumericTypes>> for EvalexprResultValue<NumericTypes> {
	fn from(value: Value<NumericTypes>) -> Self { Ok(value) }
}

impl<NumericTypes: EvalexprFloat> From<()> for Value<NumericTypes> {
...
impl<NumericTypes: EvalexprFloat> TryFrom<Value<NumericTypes>> for bool {
	type Error = EvalexprError<NumericTypes>;
...
impl<NumericTypes: EvalexprFloat> TryFrom<Value<NumericTypes>> for TupleType<NumericTypes> {
	type Error = EvalexprError<NumericTypes>;
...
impl<NumericTypes: EvalexprFloat> TryFrom<Value<NumericTypes>> for () {
	type Error = EvalexprError<NumericTypes>;
...
mod tests {
	...
	fn test_value_conversions() {
		...
		assert_eq!(Value::<DefaultNumericTypes>::from_float(3.3).as_float(), Ok(3.3));
		assert_eq!(Value::<DefaultNumericTypes>::from(true).as_boolean(), Ok(true));
		assert_eq!(Value::<DefaultNumericTypes>::from(TupleType::new()).as_tuple(), Ok(TupleType::new()));
	}
	...
	fn test_value_str_from() {
		...
		//     "string"
		// );
		assert_eq!(Value::<DefaultNumericTypes>::from_float(3.3).str_from(), "3.3");
		...
		assert_eq!(
			Value::<DefaultNumericTypes>::from(TupleType::from([
				...
				Value::<DefaultNumericTypes>::from(TupleType::from([
					Value::<DefaultNumericTypes>::from_float(42.0),
