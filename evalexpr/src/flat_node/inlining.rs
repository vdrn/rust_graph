        ...
        match source_op {
            ...
            FlatOperator::Sum { expr, variable } | FlatOperator::Product { expr, variable } => {
                ...
                if let Some(tuple_param) = pop_last_if_const_tuple(&mut new_ops)? {
                    if tuple_param.len() > 20 {
                        ...
                        if param_len == 2 {
                            ...
                            if is_sum {
                                new_ops.push(FlatOperator::AddN {
                                    n: param_len as u32,
                                });
                            } else {
                                new_ops.push(FlatOperator::MulN {
                                    n: param_len as u32,
                                });
                            }
                        }
                ...
                if is_sum {
                    ...
                    new_ops.push(FlatOperator::Sum {
                        expr: Box::new(new_expr),
                        variable: *variable,
                    });
                } else {
                    let new_expr = inline_variables_and_fold(expr, context)?;
                    new_ops.push(FlatOperator::Product {
                        expr: Box::new(new_expr),
                        variable: *variable,
                    });
                }
                ...
                if *len == 2 {
                    if let Some((a, b)) = get_last_2_if_const(&new_ops)? {
                        ...
                        if a.is_float() && b.is_float() {
                            ...
                            new_ops.push(FlatOperator::PushConst {
                                value: Value::Tuple(thin_vec![a, b]),
                            });
                        }
                    } else {
                        new_ops.push(source_op.clone());
...
fn fold_ternary_op<F: EvalexprFloat>(
    ...
    if let Some((a, b, c)) = get_last_3_if_const_as_float(new_ops)? {
        ...
        new_ops.pop();
        new_ops.push(FlatOperator::PushConst {
            value: Value::Float(fold_3(a, b, c)),
        });
    } else {
        new_ops.push(op.clone());
    }
...
fn get_last_if_const_as_float<F: EvalexprFloat>(
    ops: &[FlatOperator<F>],
) -> EvalexprResult<Option<F>, F> {
