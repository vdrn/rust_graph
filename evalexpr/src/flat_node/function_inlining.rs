...
use crate::{
    ...
    flat_node::{
        subexpression_elemination::{get_arg_ranges, get_n_previous_exprs},
        FlatOperator,
    },
    EvalexprFloat, EvalexprResult, ExpressionFunction, FlatNode, HashMapContext, IStr, Value,
};
...
fn inline_function<F: EvalexprFloat>(
    ...
    cur_idx: &mut usize,
    fn_name: IStr,
    expr_func: ExpressionFunction<F>,
    arg_num: u32,
) -> EvalexprResult<(), F> {
    ...
    *cur_idx -= num_removed_ops;
    ...
    let args = node
        .ops
        .drain(arg_range.clone())
        .collect::<Vec<FlatOperator<F>>>();
    ...
    func_expr.iter_mut(&mut |op| {
        if let FlatOperator::ReadParam { inverse_index } = op {
            *op = FlatOperator::ReadLocalVar {
                idx: shift_read_arg + (new_num_args as u32 - *inverse_index),
            };
